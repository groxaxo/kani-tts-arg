...
class NemoAudioPlayer:
    def __init__(self, config=None, text_tokenizer_name: str = None) -> None:
        ...
        if config is not None:
            ...
            self.tokeniser_length = getattr(
                config, "tokeniser_length", model_cfg.tokenizer.vocab_size
            )
            self.start_of_text = getattr(
                config, "start_of_text", model_cfg.special_tokens.start_of_text
            )
            self.end_of_text = getattr(
                config, "end_of_text", model_cfg.special_tokens.end_of_text
            )
        else:
    ...
    def get_nano_codes(self, out_ids):
        ...
        if len(audio_codes) % num_codebooks:
            raise ValueError(
                f"The length of the sequence must be a multiple of {num_codebooks}!"
            )
        audio_codes = audio_codes.reshape(-1, num_codebooks)
        audio_codes = audio_codes - torch.tensor(
            [self.codebook_size * i for i in range(num_codebooks)]
        )
...
class KaniModel:
    def __init__(
        ...
        Args:
            ...
            base_model: Optional base model name if model_name is an adapter
        """
        ...
        if config is not None:
            ...
            torch_dtype = getattr(
                config, "torch_dtype", inference_cfg.model.torch_dtype
            )
        else:
    ...
    def get_input_ids(
        self, text_promt: str, speaker_id: str = None
    ) -> tuple[torch.tensor]:
        if speaker_id is not None:
            text_promt = f"{speaker_id.strip()}: {text_promt}"
    ...
    def model_request(
        ...
        if hasattr(self, "conf") and self.conf is not None:
            max_new_tokens = getattr(
                self.conf, "max_new_tokens", inference_cfg.generation.max_new_tokens
            )
        else:
